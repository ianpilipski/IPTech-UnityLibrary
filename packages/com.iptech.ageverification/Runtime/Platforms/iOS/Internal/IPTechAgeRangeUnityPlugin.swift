...
public func IPTechAgeRangeUnityPlugin_isEligibleForAgeFeatures(callerId: Int, callback: IPTechAgeRangeUnityPlugin_isEligibleForAgeFeaturesCallback?) -> Void {
    guard let nonNilCallback = callback else {
        IPTechAgeRangeUnityPlugin.shared.logError("Callback is nil, not proceeding with call")
        return
    }
    let callbackWrapper = CallbackWrapperBool(callerId: callerId, callback: nonNilCallback)
    IPTechAgeRangeUnityPlugin.shared.isEligibleForAgeFeatures(callback: callbackWrapper)
}
    ...
    callback: IPTechAgeRangeUnityPlugin_getAgeRangeCallback?)
{
    ...
    IPTechAgeRangeUnityPlugin.shared.getAgeRange(
...
{
    let callerId: Int
    let callback: IPTechAgeRangeUnityPlugin_isEligibleForAgeFeaturesCallback
    
    private var strongSelf: CallbackWrapperBool?
    
    init(callerId: Int, callback: @escaping IPTechAgeRangeUnityPlugin_isEligibleForAgeFeaturesCallback) {
...
{
    let callerId: Int
    let callback: IPTechAgeRangeUnityPlugin_getAgeRangeCallback
    
    private var strongSelf: CallbackWrapper?
    
    init(callerId: Int, callback: @escaping IPTechAgeRangeUnityPlugin_getAgeRangeCallback) {
        self.callback = callback
        self.callerId = callerId
        self.strongSelf = self
    }
                ...
                if #available(iOS 26.0, *) {
                    do {
                        guard let viewController = getViewController() else {
                            callback.Error("[IPTechAgeRangeUnityPlugin] Could not get a UIViewController to present the AgeRangeService")
                            return;
                        }
                        ...
                        @unknown default:
                            callback.Error("[IPTechAgeRangeUnityPlugin] AgeRangeService.Response had an unhandled value in the Unknown case")
                        }
